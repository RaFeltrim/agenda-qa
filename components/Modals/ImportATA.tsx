...
import { X, FileUp, Loader2, CheckCircle2, AlertCircle, FileText, ChevronRight, File, UploadCloud } from 'lucide-react';
...
const ImportATAModal: React.FC<ImportATAModalProps> = ({ onClose, onImport }) => {
  ...
  const confirmImport = () => {
    ...
    const newCards: Card[] = preview.tasks.map((t, idx) => ({
      ...
      anexos: [],
      subTasks: [],
      historico: [{ acao: 'Card orquestrado via IA (ATA)', por: 'System', em: new Date().toISOString() }]
    }));
    onImport(newCards);
  };
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-200 border border-white/10">
        <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
           <div>
             ...
             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 ml-1">Processamento via Gemini 2.0</p>
           </div>
           ...
           {!preview ? (
             <>
               <label className="group relative border-4 border-dashed border-slate-200 dark:border-slate-700 rounded-[2rem] p-16 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 hover:bg-indigo-50/10 transition-all overflow-hidden">
                  ...
                  <p className="relative z-10 text-lg font-black dark:text-white uppercase tracking-widest">{file ? file.name : "Arraste ou Clique"}</p>
                  <p className="relative z-10 text-xs text-slate-400 font-bold mt-2 tracking-widest uppercase bg-white dark:bg-slate-800 px-3 py-1 rounded-full shadow-sm">PDF, Markdown ou TXT</p>
               </label>
               ...
               >
                 {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Processando Inteligência...</> : <><ChevronRight className="w-5 h-5" /> Extrair Tarefas com IA</>}
               </button>
             </>
           ) : (
             <div className="space-y-6 animate-in slide-in-from-right-10 duration-300">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wide border border-green-200 dark:border-green-800">{preview.tasks.length} Itens Detectados</span>
                </div>
                
                <div className="max-h-[400px] overflow-y-auto space-y-3 pr-2 scrollbar-thin">
                   {preview.tasks.map((t, i) => (
                     <div key={i} className="p-5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl hover:border-indigo-200 dark:hover:border-indigo-800 transition-colors">
                        <div className="flex justify-between items-start gap-3 mb-2">
                           <h4 className="font-bold text-sm text-slate-900 dark:text-white leading-tight">{t.titulo}</h4>
                           <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${t.urgente ? 'bg-red-500 text-white shadow-lg shadow-red-500/30' : 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400'}`}>{t.responsavel}</span>
                        </div>
                        <p className="text-[11px] text-slate-500 font-medium leading-relaxed">{t.descricao}</p>
                     </div>
                   ))}
                </div>
                
                <div className="flex gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                  <button onClick={() => setPreview(null)} className="flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-2xl font-black uppercase text-xs tracking-wider hover:bg-slate-200 transition-all">Cancelar</button>
                  <button onClick={confirmImport} className="flex-[2] py-4 bg-green-600 text-white rounded-2xl font-black uppercase text-xs tracking-wider flex items-center justify-center gap-2 hover:bg-green-700 shadow-xl shadow-green-600/20 active:scale-95 transition-all">
