...
import { Card, Space, Button, Input, Badge, Tag, Typography, Dropdown, Tabs } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined, CalendarOutlined, ClockCircleOutlined, LinkOutlined, SwapOutlined } from '@ant-design/icons';
...
// Componente de Card otimizado para mobile com botões de mover
    ...
    return (
        ...
        >
            <Space direction="vertical" size="small" className="w-full">
                <Text className="font-semibold text-base block mb-1">
                    {meeting.title}
                </Text>
                ...
                <div className="mt-3 flex items-center justify-between gap-2 flex-wrap">
                    ...
                    {/* Botão "Mover para..." - visível em mobile ou sempre disponível */}
                    {canEditContent && (isMobile || window.matchMedia('(hover: none)').matches) && (
                        <Dropdown menu={{ items: moveMenuItems }} trigger={['click']}>
                            <Button
                                size="small"
                                icon={<SwapOutlined />}
                                className="text-xs rounded-lg min-h-[32px] touch-manipulation"
                            >
                                Mover
                            </Button>
                        </Dropdown>
                                ...
                                {(provided, snapshot) => (
                                    ...
                                    >
                                        <MeetingCard
                                            ...
                                            onDelete={() => {
                                                if (confirm('Excluir reunião?')) onDelete(meeting.id);
                                            }}
...
export const KanbanBoard = () => {
    const { meetings, moveMeeting, createMeeting, deleteMeeting, setSelectedMeeting, setModalOpen, isModalOpen, selectedMeeting } = useMeetingStore();
    const { user } = useAuth();
    ...
    const canManageMeetings = userRole === 'admin';
        ...
        return {
            ...
            label: (
                <span className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full" style={{ backgroundColor: col.color }}></span>
                    {col.title}
                    <Badge count={colMeetings.length} style={{ backgroundColor: '#e2e8f0', color: '#475569', boxShadow: 'none' }} />
                </span>
            ),
            children: (
                <KanbanColumn
        ...
        <div data-testid="kanban-board" className="mt-4">
            ...
            <DragDropContext onDragEnd={onDragEnd}>
                ...
                {isMobile ? (
                    ...
                    /* Layout Desktop: Scroll horizontal com snap */
