...
const SprintListModal: React.FC<SprintListModalProps> = ({ sprints, cards, onClose, onSelectSprint, activeSprintId, onAddNewSprint }) => {
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-white/20 h-[80vh]">
        <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 ml-1">Gerenciamento de Tempo</p>
          </div>
        ...
        <div className="flex-1 p-8 overflow-y-auto space-y-4">
            <button 
              onClick={() => { onSelectSprint(null); onClose(); }}
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`p-3 rounded-2xl ${activeSprintId === null ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:bg-indigo-100 group-hover:text-indigo-600'}`}>
                   <Target className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="font-bold text-base text-slate-900 dark:text-white">Visão Global</h3>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Todas as tarefas sem filtro de sprint</p>
                </div>
              </div>
              {activeSprintId === null && <CheckCircle className="w-6 h-6 text-indigo-600 animate-in zoom-in" />}
            </button>

            <div className="flex items-center gap-4 my-6">
               <div className="h-px bg-slate-100 dark:bg-slate-800 flex-1"></div>
               <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Sprints Ativas & Planejadas</span>
               <div className="h-px bg-slate-100 dark:bg-slate-800 flex-1"></div>
            </div>
            ...
            {sprints.map((sprint) => {
              ...
              return (
                <div 
                  key={sprint.id}
                  onClick={() => { onSelectSprint(sprint.id); onClose(); }}
                ...
                >
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wide border ${
                          sprint.status === 'concluida' ? 'bg-green-100 text-green-700 border-green-200' :
                          sprint.status === 'ativa' ? 'bg-blue-100 text-blue-700 border-blue-200 animate-pulse' : 'bg-slate-100 text-slate-600 border-slate-200'
                        }`}>
                          {sprint.status}
                        </span>
                      </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-100 dark:border-slate-800">
                    <div className="flex items-center justify-between text-xs mb-3">
                      ...
                      <span className="font-black text-indigo-600 dark:text-indigo-400">{percentage}%</span>
                    </div>
