...
import { LayoutGrid, Clock, AlertCircle, BarChart3, ChevronDown, Target, PieChart, Video, CalendarPlus, X } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const stats = [
    { label: 'Total Cards', val: cards.filter(c => !activeSprintId || c.sprintId === activeSprintId).length, icon: LayoutGrid, color: 'text-indigo-600', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20' },
    { label: 'Ativos', val: cards.filter(c => c.status === 'em-progresso').length, icon: Clock, color: 'text-blue-600', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
    { label: 'Bloqueios', val: cards.filter(c => c.status === 'bloqueado').length, icon: AlertCircle, color: 'text-rose-600', bg: 'bg-rose-500/10', border: 'border-rose-500/20' },
    { label: 'Entregues', val: cards.filter(c => c.status === 'concluido').length, icon: BarChart3, color: 'text-emerald-600', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' }
  ];
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-10">
        <div className="flex-1 space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-[10px] font-black uppercase text-slate-400 leading-none mb-1.5 tracking-widest">Ciclo Atual</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {stats.map((s, i) => (
              <div key={i} className={`bg-white dark:bg-slate-800 p-5 rounded-3xl border-2 ${s.border} dark:border-slate-700/50 shadow-sm hover:shadow-md transition-all group`}>
                <div className={`${s.bg} w-10 h-10 rounded-xl flex items-center justify-center ${s.color} mb-3 group-hover:scale-110 transition-transform`}>
        ...
        <div className="lg:w-[400px] bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[2.5rem] p-7 text-white shadow-2xl shadow-indigo-600/20 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-8 relative z-10">
            <div>
              ...
              <p className="text-indigo-200 text-xs font-bold mt-1 uppercase tracking-[0.2em] opacity-80">Timeline do dia</p>
            </div>
            <button 
          ...
          <div className="space-y-4 relative z-10 max-h-[300px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
            {meetings.length === 0 ? (
              ...
              meetings.map(m => (
                <div key={m.id} className="bg-white/10 backdrop-blur-xl p-4 rounded-3xl border border-white/10 hover:bg-white/20 transition-all cursor-pointer group/item relative">
                  <button 
                  ...
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[10px] font-black bg-indigo-500 text-white px-3 py-1 rounded-full shadow-lg">{m.horario}</span>
                    <div className="flex items-center gap-1.5 text-indigo-200">
                       <Video className="w-3.5 h-3.5" />
                       <span className="text-[10px] font-bold uppercase tracking-widest">{m.local}</span>
                    </div>
                  </div>
                  <p className="text-base font-black truncate mb-3 group-hover/item:text-indigo-100 transition-colors pr-6">{m.titulo}</p>
                  <div className="flex items-center justify-between">
                    <div className="flex -space-x-2.5">
                      {m.participantes.slice(0, 3).map((p, idx) => (
                        <div key={idx} className="w-8 h-8 rounded-xl border-2 border-indigo-700 bg-indigo-500 flex items-center justify-center text-[10px] font-black shadow-md uppercase">
                          {p[0]}
                        </div>
                      ))}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-5 p-5 bg-white/50 dark:bg-slate-800/50 glass rounded-[2rem] border border-slate-200/50 dark:border-slate-700/50 shadow-sm">
        ...
        <div className="flex items-center gap-4 px-4 py-2 bg-indigo-500/5 rounded-2xl border border-indigo-500/10">
           <div className="text-right hidden sm:block">
              <p className="text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-[0.2em] leading-none mb-1">Squad Lead</p>
              <p className="text-sm font-black text-slate-800 dark:text-white">Rafael Feltrim</p>
           </div>
           <div className="relative">
              <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Rafael" className="w-11 h-11 rounded-2xl shadow-xl border-2 border-white dark:border-slate-700 object-cover" alt="Profile" />
