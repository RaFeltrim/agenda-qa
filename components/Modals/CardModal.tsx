...
import { Card, Comentario, Anexo, SubTask } from '../../types';
import { X, Calendar, User, Tag, MessageSquare, History, Paperclip, Send, Trash2, Edit2, Save, Link as LinkIcon, Clock, Plus, CheckSquare, Square, Volume2, Sparkles, Database, FileText, Loader2, Image as ImageIcon, ExternalLink, AlertCircle, Check } from 'lucide-react';
...
const CardModal: React.FC<CardModalProps> = ({ card, onClose, onUpdate, onDelete }) => {
  const [activeTab, setActiveTab] = useState<'details' | 'comments' | 'ai-help' | 'history'>('details');
  ...
  const saveChanges = () => {
    onUpdate({
      ...
      historico: [...card.historico, { acao: 'Card editado manualmente', por: 'Usuário', em: new Date().toISOString() }]
    });
    setIsEditing(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 w-full max-w-3xl max-h-[92vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-200 border border-white/10">
        
        {/* Header Profissional */}
        <div className={`px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between transition-colors ${isEditing ? 'bg-indigo-50/50 dark:bg-indigo-900/20' : 'bg-slate-50 dark:bg-slate-900/50'}`}>
          <div className="flex-1 flex items-center gap-4">
            ...
            <div className="w-full">
              <div className="flex items-center gap-2 mb-2">
                <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${card.urgente ? 'bg-red-500 text-white' : 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-400'}`}>
              ...
              {isEditing ? (
                ...
                <h2 className="text-xl font-black dark:text-white leading-tight truncate pr-4">{card.titulo}</h2>
              )}
            </div>
          </div>
          <div className="flex gap-2 flex-shrink-0 ml-4">
            {isEditing ? (
              <button onClick={saveChanges} className="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg shadow-indigo-500/30 hover:scale-105 transition-all flex items-center gap-2 px-4">
                <Save className="w-5 h-5" />
                <span className="text-xs font-black uppercase tracking-wide hidden sm:inline">Salvar</span>
              </button>
            ) : (
              <button onClick={() => setIsEditing(true)} className="p-3 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 text-slate-400 hover:text-indigo-600 shadow-sm transition-all hover:border-indigo-200">
                <Edit2 className="w-5 h-5" />
              </button>
            )}
            <button onClick={onClose} className="p-3 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 text-slate-400 hover:text-red-500 shadow-sm transition-all hover:border-red-200">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        {/* Navigation Tabs */}
        <div className="px-10 flex border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
          {[
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                   <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
                     ...
                     <span className="text-indigo-600 bg-indigo-50 dark:bg-indigo-900/50 px-2 py-1 rounded-lg">{card.subTasks.filter(s => s.concluida).length}/{card.subTasks.length}</span>
                   </h4>
                   <div className="space-y-2">
                     {card.subTasks.map(st => (
                       <div key={st.id} className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl group transition-all hover:border-indigo-200 hover:shadow-sm cursor-pointer" 
                         onClick={() => {
                            if(isEditing) return;
                            const newSub = card.subTasks.map(s => s.id === st.id ? {...s, concluida: !s.concluida} : s);
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
               <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <span className="font-black uppercase tracking-tighter">Gerar Massa de Dados</span>
                  ...
                  >
                    ...
                    <span className="font-black uppercase tracking-tighter">Relatório Estratégico</span>
               ...
               {aiLoading && (
                 <div className="flex flex-col items-center justify-center py-20 gap-4">
                   ...
                   <p className="text-sm font-black text-slate-400 uppercase tracking-widest animate-pulse">Consultando Orquestrador IA...</p>
                 </div>
               )}
               ...
               {!aiLoading && (testData || aiResult) && (
                 <div className="bg-slate-900 text-slate-100 p-8 rounded-[2.5rem] font-mono text-xs leading-relaxed overflow-x-auto shadow-2xl border border-slate-800 relative">
                    <div className="flex justify-between mb-4 border-b border-slate-800 pb-4">
                      <span className="text-indigo-400 font-bold uppercase tracking-widest flex items-center gap-2"><Sparkles className="w-4 h-4" /> IA Intelligence Report</span>
                      <button onClick={() => {setTestData(null); setAiResult(null)}} className="text-slate-500 hover:text-white transition-colors"><X className="w-4 h-4" /></button>
                    </div>
                    {testData && <pre className="whitespace-pre-wrap font-medium text-emerald-400">{testData}</pre>}
                    {aiResult && (
                      <div className="space-y-4">
                        ...
                        {aiResult.sources.length > 0 && (
                          <div className="mt-6 pt-6 border-t border-slate-800">
                             <p className="text-[10px] font-bold text-slate-500 mb-3 uppercase tracking-widest">Fontes de Pesquisa</p>
                             <div className="flex flex-wrap gap-2">
                               {aiResult.sources.map((s: any, idx: number) => (
                                 <a key={idx} href={s.web?.uri || s.maps?.uri} target="_blank" className="px-3 py-1.5 bg-slate-800 hover:bg-indigo-600 rounded-lg border border-slate-700 hover:border-indigo-500 text-indigo-400 hover:text-white transition-all flex items-center gap-2">
               ...
               <div className="relative group">
                  ...
                  <button 
                    onClick={() => {
                      ...
                      const comment: Comentario = { id: Date.now().toString(), autor: 'Rafael Feltrim', texto: newComment, timestamp: new Date().toISOString() };
               ...
               <div className="space-y-4">
                  {card.comentarios.length === 0 && (
                    <div className="text-center py-10 opacity-40">
                       ...
                       <p className="text-xs font-bold uppercase tracking-widest">Nenhum feedback registrado</p>
                    </div>
                  )}
                  {card.comentarios.map(c => (
                    <div key={c.id} className="flex gap-4 animate-in slide-in-from-bottom-2">
                       <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${c.autor}`} className="w-10 h-10 rounded-2xl border-2 border-white dark:border-slate-800 bg-slate-100 shadow-sm" alt="Avatar" />
                       <div className="flex-1 bg-white dark:bg-slate-800/50 p-5 rounded-3xl border border-slate-100 dark:border-slate-700 rounded-tl-none shadow-sm">
                          <div className="flex justify-between items-center mb-2">
                             ...
                             <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{formatTimeAgo(c.timestamp)}</span>
                          </div>
                          <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{c.texto}</p>
                       </div>
                    </div>
                  ))}
               </div>
          ...
          {activeTab === 'history' && (
             <div className="space-y-6">
                {card.historico.map((h, i) => (
                   <div key={i} className="flex gap-4 pb-6 border-l-2 border-slate-100 dark:border-slate-800 pl-6 relative last:pb-0">
                      ...
                      <div>
                         <p className="text-xs font-black text-slate-800 dark:text-slate-200">{h.acao}</p>
                         <p className="text-[10px] text-slate-400 mt-1">por <span className="font-bold text-indigo-500">{h.por}</span> • {formatTimeAgo(h.em)}</p>
                      </div>
                   </div>
                ))}
             </div>
        ...
        {/* Footer */}
        <div className="px-10 py-6 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
          <button onClick={() => onDelete(card.id)} className="flex items-center gap-2 text-red-500 hover:text-red-700 text-[10px] font-black uppercase tracking-widest transition-all hover:bg-red-50 px-4 py-2 rounded-xl">
            <Trash2 className="w-4 h-4" /> Deletar Card
          </button>
          <div className="flex gap-4">
             {isEditing && (
               <button onClick={saveChanges} className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-indigo-700 shadow-lg shadow-indigo-500/20 transition-all">
                 Salvar Alterações
               </button>
             )}
             <button onClick={onClose} className="px-8 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-slate-100 transition-all">
               Fechar
             </button>
          </div>
        </div>
