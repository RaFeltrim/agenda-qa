...
function initializeEnv(): EnvConfig {
  ...
  // CRITICAL: Throw error for missing required vars (in ALL environments)
  // This prevents the app from running with broken configuration
  if (!validation.isValid) {
        ...
        // In production, ALWAYS throw to prevent silent failures
        if (import.meta.env.PROD) {
                  throw new Error(errorMsg);
        }

        // In development, warn but continue with empty strings
        console.error('[ENV] 🚨 Application may not function correctly!');
  }
