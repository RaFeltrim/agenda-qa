...
function handleApiError(error: any): never {
  ...
  if (message.includes("permission") || message.includes("403")) {
    throw new Error("Erro de Permissão (403): O projeto ou a chave não tem acesso ao modelo solicitado (Pro). Certifique-se de que o faturamento está ativo no console do Google Cloud.");
  }
  ...
  if (message.includes("Requested entity was not found") || message.includes("404")) {
    ...
    throw new Error("Configuração expirada ou modelo indisponível. Re-selecionando chave de API...");
  }
  
  throw error;
}

export async function extractTasksFromDocument(content: string, mimeType: string = "text/plain"): Promise<ExtractedTasks> {
  try {
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    
    const contents = mimeType === "application/pdf" 
      ? { parts: [{ inlineData: { data: content, mimeType: "application/pdf" } }, { text: "Extraia as tarefas desta ATA conforme o schema JSON." }] }
