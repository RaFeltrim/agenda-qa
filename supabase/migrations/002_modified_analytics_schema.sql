-- Modified Analytics schema for existing Agenda-QA database structure
-- This version adapts the analytics to work with your current table structure

-- Create schema for analytics (if not exists)
CREATE SCHEMA IF NOT EXISTS analytics;

-- Materialized view for team velocity metrics - adapted for existing schema
CREATE MATERIALIZED VIEW IF NOT EXISTS analytics.mv_team_velocity AS
WITH sprint_data AS (
  SELECT 
    COALESCE(s.team_id, '00000000-0000-0000-0000-000000000000') as team_id,
  ...
  GROUP BY s.team_id, t.name, s.id, s.name, s.start_date, s.end_date
)
SELECT 
  sd.team_id,
  sd.team_name,
...
HAVING COUNT(*) >= 1  -- Need at least 1 sprint for meaningful metrics
...
-- Materialized view for sprint burndown data - adapted for existing schema
...
daily_progress AS (
  ...
  GROUP BY sc.sprint_id, sc.sprint_name, sc.team_id, sc.day_date, sc.velocity_goal, sc.start_date, sc.end_date
)
SELECT 
  dp.sprint_id,
  dp.sprint_name,
...
LEFT JOIN public.teams t ON dp.team_id = t.id
...
-- Materialized view for user productivity metrics - adapted for existing schema
CREATE MATERIALIZED VIEW IF NOT EXISTS analytics.mv_user_productivity AS
WITH user_activities AS (
  SELECT 
    COALESCE(p.id, al.changed_by) as user_id,
    COALESCE(p.full_name, 'Unknown User') as user_name,
    COALESCE(p.username, p.full_name, 'unknown') as email,
  ...
  GROUP BY p.id, p.full_name, p.username, DATE_TRUNC('week', al.created_at)
),
weekly_stats AS (
  SELECT 
    ua.user_id,
...
SELECT 
  ...
  -- Ranking within team (approximate)
  ROW_NUMBER() OVER (
...
-- Materialized view for quality metrics - adapted for existing schema
...
WITH task_lifecycle AS (
  SELECT 
    ...
    c.tags
  FROM public.cards c
  LEFT JOIN public.sprints s ON c.sprint_id = s.id
  WHERE c.created_at >= CURRENT_DATE - INTERVAL '180 days'
),
team_quality AS (
  SELECT 
    COALESCE(tl.team_id, '00000000-0000-0000-0000-000000000000') as team_id,
    COALESCE(t.name, 'Unassigned') as team_name,
  ...
  LEFT JOIN public.teams t ON tl.team_id = t.id
...
-- Create indexes for performance on existing tables
