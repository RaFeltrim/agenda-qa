...
const AuditLogDrawer: React.FC<AuditLogDrawerProps> = ({ cards, onClose }) => {
  const allHistory = cards.flatMap(card => 
      ...
      <div className="relative w-full max-w-md h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-l border-white/20">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold tracking-wider mt-1">Rastreabilidade Completa</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          {allHistory.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 opacity-40">
              ...
              <p className="text-sm font-black uppercase tracking-widest text-slate-400">Sem registros</p>
            </div>
          ) : (
            allHistory.map((h, i) => (
              <div key={i} className="group relative pl-8 border-l-2 border-slate-100 dark:border-slate-800 pb-2">
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 group-hover:border-indigo-200 dark:group-hover:border-indigo-800 transition-all shadow-sm">
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-slate-200/50 dark:border-slate-700/50">
                    ...
                    <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 truncate bg-white dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-100 dark:border-slate-700">{h.cardTitle}</span>
                  </div>
                </div>
              </div>
            ))
