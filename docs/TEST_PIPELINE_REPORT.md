...
**Supabase:** Projeto `YOUR_PROJECT_ID`
...
O projeto Supabase atual (`YOUR_PROJECT_ID`) **NÃO possui usuários confirmados**. Foi criado o usuário `test.e2e@example.com` via API de signup, porém a confirmação de email está **habilitada** no Supabase e o email não foi confirmado. **Todas as falhas em testes autenticados** são decorrentes deste único bloqueio.
...
- **Detalhes:** Usuário `test.e2e@example.com` foi criado via API, mas Supabase requer confirmação de email (`email_not_confirmed`)
