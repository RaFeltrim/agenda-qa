...
const FinishSprintModal: React.FC<FinishSprintModalProps> = ({ sprint, cards, nextSprints, onClose, onConfirm }) => {
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-white/10">
        <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1 ml-1">Review & Retrospectiva</p>
          </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-5 bg-emerald-50 dark:bg-emerald-900/10 rounded-[2rem] border border-emerald-100 dark:border-emerald-900/30">
              ...
              <p className="text-4xl font-black text-emerald-700 dark:text-emerald-400 tracking-tighter">{doneCards.length}</p>
            </div>
            <div className="p-5 bg-amber-50 dark:bg-amber-900/10 rounded-[2rem] border border-amber-100 dark:border-amber-900/30">
              ...
              <p className="text-4xl font-black text-amber-700 dark:text-amber-400 tracking-tighter">{pendingCards.length}</p>
            </div>
          </div>
          ...
          {pendingCards.length > 0 && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                >
                  <div className={`p-3 rounded-xl transition-colors ${destination === 'backlog' ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:bg-slate-200'}`}>
                    <Archive className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="font-bold text-sm text-slate-900 dark:text-white">Mover para o Backlog</p>
                    <p className="text-[10px] text-slate-500 font-medium">As tarefas ficarão sem sprint definida.</p>
                  </div>
                </button>

                {nextSprints.length > 0 && nextSprints.map(next => (
                  <button
                    key={next.id}
                  ...
                  >
                    <div className={`p-3 rounded-xl transition-colors ${destination === next.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:bg-slate-200'}`}>
                      <Forward className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="font-bold text-sm text-slate-900 dark:text-white">Mover para {next.nome}</p>
                      <p className="text-[10px] text-slate-500 font-medium">Carryover para o próximo ciclo.</p>
                    </div>
                  </button>
                ))}
              </div>
